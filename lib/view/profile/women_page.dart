...
class Women_Products extends StatefulWidget {
  const Women_Products({super.key});

  @override
  State<Women_Products> createState() => _Women_ProductsState();
}

class _Women_ProductsState extends State<Women_Products> {
  late Future<List<Product>> allProducts;
  ...
  Future<List<Product>> fetchProducts() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Product> products = data.map((item) => Product.fromJson(item))
          .where((product) => product.category == 'women\'s clothing') // Filter for men's clothing
          .toList();
      setState(() {
